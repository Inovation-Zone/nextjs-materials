...
const General = () => {
  ...
  const aboutContentViEditorRef = useRef<any>(null);
  const aboutContentEnEditorRef = useRef<any>(null);
  ...
  useEffect(() => {
    settings.forEach((item: Setting) => {
      if (item?.key === 'aboutContentVi') {
        aboutContentViEditorRef.current.setValue(item?.value);
      }
      if (item?.key === 'aboutContentEn') {
        aboutContentEnEditorRef.current.setValue(item?.value);
      }
    });
  }, [settings]);
  ...
  const handleSaveSettings = () => {
    const settings = [
      {
        key: 'aboutContentVi',
        value: aboutContentViEditorRef.current.getValue()
      },
      {
        key: 'aboutContentEn',
        value: aboutContentEnEditorRef.current.getValue()
      },
    ]
  ...
  return (
    <Space
      ...
      <Breadcrumb>
        ...
        <Breadcrumb.Item>{translate.setting.about.title}</Breadcrumb.Item>
      </Breadcrumb>
      <Typography.Title level={2}>{translate.setting.about.title}</Typography.Title>
      <Col className="flex flex-col gap-3">
        <Col className="flex flex-col gap-2">
          <Typography className="font-md">{translate.common.content} (Vietnamese)</Typography>
          <Editor ref={aboutContentViEditorRef} />
        </Col>
        <Col className="flex flex-col gap-2">
          <Typography className="font-md">{translate.common.content} (English)</Typography>
          <Editor ref={aboutContentEnEditorRef} />
        </Col>
      </Col>
      <Divider />
